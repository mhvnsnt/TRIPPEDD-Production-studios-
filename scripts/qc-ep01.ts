...
if (failures.length) {
  await fs.writeFile(qcPath, JSON.stringify({ episodeId: 'EP01', status: 'FAILED', failures, checkedAt: new Date().toISOString(), checks: { firstAssembly: firstProbe, subjectivityAsset: subjectivityProbe, showrunner: showrunnerProbe, vmaf } }, null, 2));
  console.error(JSON.stringify({ status: 'FAILED', failures }, null, 2)); process.exitCode = 2;
} else {
  const result = { episodeId: 'EP01', status: 'PASS', checkedAt: new Date().toISOString(), checks: { firstAssembly: firstProbe, subjectivityAsset: subjectivityProbe, showrunner: showrunnerProbe, vmaf, editorialLockPresent: true }, note: 'Technical/media QC passed. This does not constitute showrunner greenlight.' };
  await fs.writeFile(qcPath, JSON.stringify(result, null, 2)); console.log(JSON.stringify(result, null, 2));
}
