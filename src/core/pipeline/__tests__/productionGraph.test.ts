...
describe('Production Graph (Event Ledger & Approvals Layer)', () => {
  ...
  it('1. Every blocked transition produces a PRODUCTION_BLOCKED event.', () => {
    const unit: ProductionUnit = { id: 'u1', type: 'LIVE_ACTION_SKETCH', name: 'Joe Reconstruction', description: '', status: 'DEVELOPMENT' };
  ...
  it('2. Every successful transition produces a PRODUCTION_ADVANCED event.', () => {
    const unit: ProductionUnit = { id: 'u2', type: 'LIVE_ACTION_SKETCH', name: 'Test Unit', description: '', status: 'DEVELOPMENT' };
    graph.addProductionUnit(unit);
  ...
  it('3. Requirement satisfaction produces a REQUIREMENT_SATISFIED event.', () => {
    const unit: ProductionUnit = { id: 'u3', type: 'LIVE_ACTION_SKETCH', name: 'Test Unit 3', description: '', status: 'DEVELOPMENT' };
    graph.addProductionUnit(unit);
  ...
  it('4. Events are append-only (no deletion method exists).', () => {
    const unit: ProductionUnit = { id: 'u4', type: 'LIVE_ACTION_SKETCH', name: 'Test Unit 4', description: '', status: 'DEVELOPMENT' };
    graph.addProductionUnit(unit);
  ...
  it('6. "Why is this blocked?" returns the actual causal dependency chain.', () => {
    const unit: ProductionUnit = { id: 'u6', type: 'LIVE_ACTION_SKETCH', name: 'Test Unit 6', description: '', status: 'DEVELOPMENT' };
    graph.addProductionUnit(unit);
