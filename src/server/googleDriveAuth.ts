...
function challenge(verifier: string) { return base64Url(crypto.createHash('sha256').update(verifier).digest()); }

async function loadTokens(): Promise<StoredTokens | null> {
  if (cachedTokens) return cachedTokens;
  try { cachedTokens = JSON.parse(await fs.readFile(TOKEN_FILE, 'utf8')) as StoredTokens; return cachedTokens; } catch { return null; }
}
...
export function createGoogleAuthorizationUrl() {
  ...
  const params = new URLSearchParams({
    ...
    scope: DRIVE_SCOPE, state, code_challenge: challenge(verifier), code_challenge_method: 'S256',
  });
...
export async function exchangeGoogleCode(code: string, state: string) {
  ...
  if (!pending || pending.expiresAt < Date.now()) throw new Error('Google OAuth state is missing or expired.');
  const body = new URLSearchParams({ code, client_id: clientId(), redirect_uri: redirectUri(), grant_type: 'authorization_code', code_verifier: pending.verifier });
  ...
  await saveTokens({ access_token: payload.access_token, refresh_token: payload.refresh_token || previous?.refresh_token, expires_at: Date.now() + Number(payload.expires_in || 3600) * 1000, scope: payload.scope, token_type: payload.token_type });
}
...
export async function getGoogleAccessToken(): Promise<string | null> {
  ...
  const body = new URLSearchParams({ client_id: clientId(), refresh_token: tokens.refresh_token, grant_type: 'refresh_token' });
  const response = await fetch(GOOGLE_TOKEN_URL, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body });
  const payload = await response.json() as any;
...
export async function revokeGoogleAccess() {
  const tokens = await loadTokens();
  if (tokens?.refresh_token) await fetch(`https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(tokens.refresh_token)}`, { method: 'POST' });
  cachedTokens = null;
