...
import { listPublicDriveFolder, downloadPublicDriveFile, DEFAULT_PUBLIC_DRIVE_FOLDER_URL } from "./src/server/publicDriveFolder";
...
const DEFAULT_DRIVE_FOLDER_URL = process.env.TRIPPEDD_DRIVE_FOLDER_URL || DEFAULT_PUBLIC_DRIVE_FOLDER_URL;
...
function driveError(status: number, detail: string, publicMode: boolean) {
  if (publicMode && (status === 400 || status === 401 || status === 403)) return `Public Drive API access failed (${status}): ${detail}`;
...
async function startServer() {
  ...
  const scanDriveFolder = async (folderId: string, token?: string) => {
    ...
    if (!token && !apiKey) {
      console.log('[Drive] No API key/OAuth configured. Using credential-free public-folder downloader.');
      const entries = await listPublicDriveFolder(DEFAULT_DRIVE_FOLDER_URL);
      const cacheDir = path.join(process.cwd(), '.trippedd', 'public-drive');
      await fs.mkdir(cacheDir, { recursive: true });
      let newCount = 0;
      for (const entry of entries) {
        const safeName = `${entry.id.replace(/[^a-zA-Z0-9_-]/g, '_')}${path.extname(entry.name)}`;
        const localPath = path.join(cacheDir, safeName);
        try { await fs.access(localPath); }
        catch { console.log(`[Drive] Downloading public media: ${entry.name}`); await downloadPublicDriveFile(entry.url, localPath); }
        if (!queueManager.getJob(entry.id)) {
          queueManager.addJob({ id: 'JOB_' + entry.id, fileId: entry.id, originalName: entry.name, mimeType: entry.mimeType, state: 'QUEUED', progress: 0, logs: ['Discovered from public Drive link.', 'Access mode: credential-free (gdown).'], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), tools: {}, evidenceRefs: [] } as any);
          newCount++;
        }
        queueManager.setLocalSource(entry.id, localPath);
      }
      return { success: true, total: entries.length, discovered: entries.length, ignoredNonMedia: 0, new: newCount, accessMode: 'credential-free-public-link', downloaded: entries.length };
    }
    ...
    const files: any[] = [];
    do {
      const params = new URLSearchParams({ q: `'${folderId}' in parents and trashed = false`, fields: 'nextPageToken,files(id,name,mimeType,size,md5Checksum,thumbnailLink,videoMediaMetadata)', pageSize: '1000', orderBy: 'name_natural' });
      ...
      if (!driveRes.ok) throw new Error(driveError(driveRes.status, await driveRes.text(), publicMode));
    ...
    for (const f of mediaFiles) {
      if (!queueManager.getJob(f.id)) {
        queueManager.addJob({ id: 'JOB_' + f.id, fileId: f.id, originalName: f.name, mimeType: f.mimeType, size: f.size, hash: f.md5Checksum, thumbnailLink: f.thumbnailLink, state: 'QUEUED', progress: 0, logs: ['Discovered in Drive scan.', publicMode ? 'Access mode: public-link.' : 'Access mode: OAuth.'], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), tools: {}, evidenceRefs: [] } as any);
        newCount++;
      }
      queueManager.setAccessToken(f.id, token ? token : `public:${apiKey}`);
    }
    return { success: true, total: mediaFiles.length, discovered: files.length, ignoredNonMedia: files.length - mediaFiles.length, new: newCount, accessMode: publicMode ? 'public-link' : 'oauth' };
  };

  app.get('/api/auth/google/status', async (_req, res) => { res.json({ ...getGoogleOAuthStatus(), credentialFreePublicDrive: true, publicFolderUrl: DEFAULT_DRIVE_FOLDER_URL }); });
  ...
  app.post('/api/queue/scan', async (req, res) => { try { const folderId = String(req.body.folderId || DEFAULT_DRIVE_FOLDER_ID); const token = await getGoogleAccessToken(); res.json(await scanDriveFolder(folderId, token || undefined)); } catch (e: any) { res.status(500).json({ error: e.message }); } });
    ...
    try {
      ...
      if (token) queueManager.setAccessToken(req.params.fileId, token);
        ...
        if (key) queueManager.setAccessToken(req.params.fileId, `public:${key}`);
          ...
          const match = files.find(name => name.startsWith(req.params.fileId));
          if (match) queueManager.setLocalSource(req.params.fileId, path.join(cacheDir, match));
        }
      }
      const ok = queueManager.retry(req.params.fileId); if (ok) res.json({ success: true }); else res.status(404).json({ error: 'Job not found or no source media is available.' });
