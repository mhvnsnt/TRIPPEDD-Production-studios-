...
def character(frame,idx,shot):
 p=TEENS[idx];lean,head_y,left_a,right_a,head_x=pose(frame,shot,idx);phase=frame*(.35+idx*.035)+idx*55;lean=lean*p["lean"]+math.sin(math.radians(phase))*(2.5 if shot in {"S01","S05"} else 1.0)
 if idx==0:right_a+=5;head_x+=3
 elif idx==1:left_a*=.9;right_a*=.9;head_x-=3
 else:left_a*=.78;right_a*=.78;head_x-=6
 stride=math.sin(math.radians(phase))*(18+idx*3) if shot in {"S04","S08","S09"} else 0
 left_a-=stride*.55;right_a+=stride*.55;body_h=155;hand_l=point(-4,55,left_a,92);hand_r=point(4,55,right_a,92);foot_l=point(lean*.3,body_h,-38-stride,92);foot_r=point(lean*.3,body_h,38+stride,92)
