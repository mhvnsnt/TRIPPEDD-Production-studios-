#!/usr/bin/env python3
"""Durable webhook/reconciliation supervisor for the TRIPPEDD JIT runner.

Webhook delivery accelerates provisioning; reconciliation remains authoritative.
"""
from __future__ import annotations
import hashlib,hmac,json,os,pathlib,subprocess,threading,time,urllib.request
...
def load():
    ...
    except FileNotFoundError:return {"active_jobs":{}}
...
def provision(job):
    ...
    with LOCK:
        ...
        save(s)
        event("provision_requested",{"job_id":jid,"run_id":job.get("run_id")})
    env=os.environ.copy()
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        # Provision only queued jobs targeting the production label.
