...
def build_receipt(repo_root: Path, receipt: Path, blend: Path, png: Path, frame: int, version: str) -> dict:
    ...
    if receipt.parent not in png.parents:
        raise ValueError("rendered PNG must be stored under the receipt directory")
    return {
        "schema": SCHEMA,
        "rendered": True,
        "shot_id": SHOT_ID,
        ...
        "frame": frame,
        "artifact": {
            "path": str(png.relative_to(receipt.parent)),
...
def main() -> int:
    ...
    parser.add_argument("--blend", type=Path, required=True, help="existing resolved Blender scene")
    ...
    try:
        ...
        if not blend.is_file() or blend.suffix.lower() != ".blend":
            raise RuntimeError(f"resolved Blender scene does not exist or is not .blend: {blend}")
