/**
 * THE EVIDENCE QUEUE — the provisioner/governor/analyzer lane.
 *
 * This is NOT a rival of src/server/queueManager.ts and must not be merged into
 * it by hand. They are different architectures that happen to share a name:
 *   queueManager.ts   drives AutonomousStudioOrchestrator / studioPlan /
 *                     productionMemory — the live app path.
 *   evidenceQueue.ts  drives ToolProvisioner / ResourceGovernor /
 *                     ArtifactLifecycle / the analyzer set, and is what the
 *                     execution-integrity tests hold to account.
 *
 * Kept whole and separate on purpose. Two things in here are the fix for the
 * most expensive bug class in this project and exist in NO other lane:
 *   - the provisioner-ready gate, so a job cannot run before tool detection
 *     finishes and report every analyzer NOT_INSTALLED as though the clip were
 *     silent;
 *   - evidence persistence, so ~40 minutes of CPU does not evaporate on a
 *     container restart.
 * Whoever unifies the two lanes has to carry both across or say why not.
 */
