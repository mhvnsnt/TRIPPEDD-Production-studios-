...
export interface PilotAssemblyClip {
  sourceFileId: string;
  sourcePath: string;
  start: number;
  end: number;
...
export interface PilotAssemblyManifest {
  episodeId: 'EP01';
  title: 'The Walk';
  status: 'ROUGH_CUT_READY' | 'WAITING_FOR_EVIDENCE';
  generatedAt: string;
  sourceClipCount: number;
  selectedClipCount: number;
  clips: PilotAssemblyClip[];
  missingBeats: string[];
  outputPath?: string;
}

const CACHE_ROOT = path.resolve(process.env.TRIPPEDD_MEDIA_CACHE || path.join(process.cwd(), '.trippedd', 'media'));
const OUTPUT_ROOT = path.resolve(process.env.TRIPPEDD_OUTPUT_DIR || path.join(process.cwd(), '.trippedd', 'renders'));
...
export async function buildEp01FirstAssembly(options: { maxClips?: number; clipPaddingSeconds?: number } = {}): Promise<PilotAssemblyManifest> {
  ...
  const sources = Object.values(memory.sources) as Array<{ fileId?: string; mediaPath?: string; name?: string; analysis?: any }>;
  const gags = Object.values(memory.gags) as Array<any>;

  const sourcePaths = new Map<string, string>();
  for (const source of sources) {
    if (source.fileId && source.mediaPath && safeCachedPath(source.mediaPath)) sourcePaths.set(source.fileId, source.mediaPath);
  }
  ...
  const missingBeats = selected.length === 0
    ? ['MEDIA_ANALYSIS', 'GAG_DISCOVERY', 'SOURCE_SELECTS']
    : ['STORY_REVIEW', 'SUBJECTIVITY_GENERATION', 'FINAL_EDITORIAL_ASSEMBLY', 'QC', 'GREENLIGHT'];

  const manifest: PilotAssemblyManifest = {
    episodeId: 'EP01', title: 'The Walk', status: selected.length ? 'ROUGH_CUT_READY' : 'WAITING_FOR_EVIDENCE',
    generatedAt: new Date().toISOString(), sourceClipCount: sourcePaths.size, selectedClipCount: selected.length,
    clips: selected, missingBeats,
  };
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    await run('ffmpeg', [
      '-y', '-hide_banner', '-loglevel', 'error', '-ss', clip.start.toFixed(3), '-i', clip.sourcePath,
      '-t', (clip.end - clip.start).toFixed(3), '-map', '0:v:0', '-map', '0:a?',
