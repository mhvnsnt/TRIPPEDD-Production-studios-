...
POSES={
    "hang":(0,-110,-25,25,0),"talk":(1,-108,-22,20,2),"turn":(-7,-112,-28,30,-8),
    "stare":(-10,-100,-22,34,-12),"panic":(-12,-140,-45,45,-5),"grab":(-10,-48,-28,34,5),
    "run":(-18,-125,-42,42,14),"duck":(-8,-90,-30,30,0),"hide":(-4,-65,-25,25,-8),
    "peek":(-12,-70,-30,28,-20),"crouch":(2,-75,-24,24,0),"breathe":(0,-78,-24,24,0),
    "look_pack":(1,-55,-24,28,8),"look_bush":(0,-75,-22,30,26),"ask":(0,-118,-22,24,-4),
    "glance_bush":(0,-75,-22,28,26),"neutral":(0,-105,-25,25,0),"point":(0,-90,-25,20,18),
    "commit":(-8,-115,-38,42,24),"throw":(-20,-150,-45,48,18),"flee":(-20,-145,-45,45,18),
    "flee_fast":(-24,-155,-52,52,22)
}
SEQUENCES={
    "S01":(0,83,["hang","talk","hang"]),"S02":(108,143,["hang","turn","stare","panic"]),
    "S03":(144,215,["panic","grab","turn","run"]),"S04":(216,299,["run","duck","hide","peek"]),
    "S05":(300,371,["crouch","breathe","look_pack","look_bush"]),"S06":(372,419,["ask","glance_bush"]),
    "S07":(420,455,["neutral","point","commit"]),"S08":(456,539,["run","throw","flee"]),
...
def pose(frame,shot,idx):
    ...
    start,end,names=SEQUENCES[shot];p=TEENS[idx]
    local=(frame-start-p["delay"])*p["tempo"]
    t=max(0,min(1,local/max(1,end-start)))
    u=ease(t)*(len(names)-1)
    i=min(len(names)-2,int(u)) if len(names)>1 else 0
    q=u-i if len(names)>1 else 0
    a=POSES[names[i]];b=POSES[names[i+1]] if len(names)>1 else a
...
def character(frame,idx,shot):
    ...
    phase=frame*(.35+idx*.035)+idx*55
    active=shot in {"S01","S05","S06","S07"}
    travel=shot in {"S04","S08","S09"}

    # Micro-acting: breath, anticipation and recovery sit underneath the main pose.
    breath=math.sin(math.radians(phase*1.7))* (3.0 if active else 1.2)
    lean=lean*p["lean"] + breath
    if travel:
        stride=math.sin(math.radians(phase))*(18+idx*3)
    else:
        stride=0

    # Each performer has a different reaction delay and head response.
    if shot=="S02":
        response=max(0,min(1,(frame-(108+idx*5))/18))
        head_x += math.sin(response*math.pi)*(-10-idx*2)
        head_y += math.sin(response*math.pi)*(-3)
    elif shot=="S03":
        response=max(0,min(1,(frame-(144+idx*4))/22))
        head_x += math.sin(response*math.pi)*(-5)
    elif shot=="S05":
        head_y += breath*.45
    elif shot=="S06":
        head_x += math.sin(math.radians(phase*1.25))*3
    elif shot=="S07":
        head_x += math.sin(math.radians(phase*.9))*2
    ...
    # Small hand anticipation/recovery beats make dialogue and decisions readable.
    if shot=="S06":
        beat=math.sin(math.radians((frame-372)*12+idx*30))
        right_a+=beat*4
    if shot=="S07":
        prep=clamp((frame-(420+idx*3))/18,0,1)
        right_a-=prep*10
    if shot=="S08":
    ...
    body_h=155
    hand_l=point(-4,55,left_a,92);hand_r=point(4,55,right_a,92)
    foot_l=point(lean*.3,body_h,-38-stride,92);foot_r=point(lean*.3,body_h,38+stride,92)

    # Eye language is deliberately simple: gaze shifts rather than adding a
    # complicated face rig. The pupils move a few pixels toward the active beat.
    gaze_x=0;gaze_y=0
    ...
    blink_phase=(frame+idx*17)%53
    blink=1 if blink_phase in {51,52} else 0
    eye_r=1.2 if blink else 5
    mouth=""
    ...
    eyes=(f"<circle cx='-10' cy='{-5+gaze_y:.2f}' r='{eye_r}' fill='#0b0d12' stroke='none'/>"
          f"<circle cx='{10+gaze_x:.2f}' cy='{-5+gaze_y:.2f}' r='{eye_r}' fill='#0b0d12' stroke='none'/>")
...
def performance_layer(frame,shot):
