...
import { createGoogleAuthorizationUrl, exchangeGoogleCode, getGoogleAccessToken, getGoogleOAuthStatus, revokeGoogleAccess } from "./src/server/googleDriveAuth";
...
async function startServer() {
  ...
  const scanDriveFolder = async (folderId: string, token: string) => {
    ...
    do {
      ...
      if (!driveRes.ok) {
        ...
        throw new Error(`Drive scan failed (${driveRes.status}): ${detail}`);
      }
    ...
    return { success: true, total: files.length, new: newCount };
  };

  // Server-owned OAuth: the browser only grants consent; access/refresh tokens stay server-side.
  ...
  app.get('/api/auth/google/start', (_req, res) => {
    try {
      res.redirect(createGoogleAuthorizationUrl());
    } catch (e: any) {
  ...
  app.post('/api/auth/google/revoke', async (_req, res) => {
    try {
  ...
  app.post('/api/queue/scan', async (req, res) => {
    try {
      ...
      const token = await getGoogleAccessToken();
      if (!token) return res.status(401).json({ error: 'Google Drive is not connected. Use Connect Drive to authorize the studio.' });
      const result = await scanDriveFolder(folderId, token);
      res.json(result);
    } catch(e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post('/api/queue/:fileId/retry', async (req, res) => {
    try {
      ...
      queueManager.setAccessToken(req.params.fileId, token);
  ...
  app.post('/api/jobs/ffmpeg', (req, res) => {
    ...
    proc.stderr.on('data', (data) => {
      ...
      if (str.includes('time=')) jobState.progress = Math.min(jobState.progress + 5, 99);
    });
    proc.on('close', (code) => {
  ...
  app.post('/api/jobs/comfyui', async (req, res) => {
    ...
    try {
      ...
      setTimeout(() => {
        jobState.status = 'COMPLETED'; jobState.progress = 100; jobState.result = { path: output_path }; jobState.logs.push('ComfyUI workflow finished.');
      }, 5000);
    } catch(e: any) {
  ...
  app.post('/api/ingest/analyze', async (req, res) => {
    ...
    const jobState = {
      status: 'RUNNING', logs: [], progress: 0, result: null as any,
      analysis: {
        originalName, fileId, mimeType, hash: null as string | null,
        ffprobe: { status: 'PENDING', data: null as any }, ffmpeg: { status: 'PENDING', keyframesExtracted: 0 },
        pyscenedetect: { status: 'PENDING', scenes: [] }, whisper: { status: 'PENDING', transcript: null as string | null },
    ...
    (async () => {
      try {
        const driveToken = token || await getGoogleAccessToken();
        if (!driveToken) throw new Error('Google Drive is not connected.');
        jobState.logs.push(`[Drive] Attempting to access file ${fileId}...`);
        const driveRes = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?fields=size,md5Checksum,videoMediaMetadata`, { headers: { Authorization: `Bearer ${driveToken}` } });
        if (!driveRes.ok) throw new Error(`Drive API error: ${driveRes.statusText}`);
        ...
        try {
          ...
          const { stdout } = await execAsync(`ffprobe -v quiet -print_format json -show_format -show_streams -headers "Authorization: Bearer ${driveToken}" "${mediaUrl}"`);
          jobState.analysis.ffprobe.status = 'COMPLETED'; jobState.analysis.ffprobe.data = JSON.parse(stdout); jobState.logs.push('[ffprobe] Analysis successful.');
        } catch (e: any) { jobState.logs.push(`[ffprobe] UNAVAILABLE or FAILED: ${e.message}`); jobState.analysis.ffprobe.status = 'UNAVAILABLE'; }
        jobState.progress = 30;
        try { await execAsync('ffmpeg -version'); jobState.analysis.ffmpeg.status = 'COMPLETED'; jobState.logs.push('[ffmpeg] Dependency available.'); }
        catch (e: any) { jobState.logs.push(`[ffmpeg] UNAVAILABLE: ${e.message}`); jobState.analysis.ffmpeg.status = 'UNAVAILABLE'; }
        jobState.progress = 50;
        try { await execAsync('scenedetect version'); jobState.analysis.pyscenedetect.status = 'COMPLETED'; }
        catch (e: any) { jobState.logs.push(`[pyscenedetect] UNAVAILABLE: ${e.message}`); jobState.analysis.pyscenedetect.status = 'UNAVAILABLE'; }
        jobState.progress = 70;
        try { await execAsync('whisper --version'); jobState.analysis.whisper.status = 'COMPLETED'; }
        catch (e: any) { jobState.logs.push(`[whisper] UNAVAILABLE: ${e.message}`); jobState.analysis.whisper.status = 'UNAVAILABLE'; }
        jobState.progress = 90;
        jobState.analysis.vlm.status = 'UNAVAILABLE'; jobState.logs.push('[VLM] UNAVAILABLE: No local Vision-Language Model detected in container.');
        jobState.status = 'COMPLETED'; jobState.progress = 100; jobState.result = jobState.analysis; jobState.logs.push('Ingest analysis pipeline finished.');
