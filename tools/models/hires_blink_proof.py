...
RES = int(opt("--res", "760"))
N   = int(opt("--frames", "7"))
OUT = os.path.join(ROOT, "docs", "evidence", "hires_blink")
os.makedirs(OUT, exist_ok=True)
SCN = os.path.join(ROOT, "assets/rigs/MARS_FACE_HIRES.blend")
...
blinks = [k for k in keys if "blink" in k.lower()]
if not blinks: die("the cage carries no blink shape key")
...
if os.path.exists(lw):
    ...
    LID = np.vstack([np.array(S3[k]) for k in
                     ("eyelid_R_upper", "eyelid_L_upper") if k in S3])
    src = "the owner's own drawn upper-lid lines"
else:
...
for i in range(N):
    ...
    series.append({"frame": i, "drive": round(v, 3),
                   "lidTravelMeanMM": round(float(travel.mean()), 3),
                   "lidTravelMaxMM": round(float(travel.max()), 3)})
    print("  f%02d drive %.2f  lid travel mean %6.2f mm  max %6.2f mm"
          % (i, v, travel.mean(), travel.max()), flush=True)
drive(0.0)

peak = max(s["lidTravelMaxMM"] for s in series)
ends = series[0]["lidTravelMaxMM"], series[-1]["lidTravelMaxMM"]
if peak < 1.0:
    die("the render mesh's lid moves at most %.2f mm across the whole arc -- it is "
        "not following the blink, whatever the cage does" % peak)
...
json.dump({"scene": os.path.relpath(SCN, ROOT), "renderMesh": "MARS_RENDER",
           "renderVerts": int(len(base)), "trackedFrom": src,
           "trackedVerts": int(len(track)), "frames": frames, "series": series,
           "peakLidTravelMM": round(peak, 3)},
          open(os.path.join(OUT, "blink_series.json"), "w"), indent=2)
print("\npeak lid travel on the RENDER mesh: %.2f mm over %d frames" % (peak, N), flush=True)
print("-> docs/evidence/hires_blink/", flush=True)
