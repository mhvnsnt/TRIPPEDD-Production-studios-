...
This is deliberately a pre-render gate, not a renderer and not evidence.
It resolves the typed shot contract against materialized local assets and
refuses execution when the canonical Mars payload or environment is absent.

Usage:
  python ops/autonomous-studio/first_shot_preflight.py [manifest.json]

Exit codes:
  0 = RESOLVED; execution may proceed
  2 = BLOCKED; required production input is missing/invalid
"""
...
def main() -> int:
    ...
    if not all(contract.get(k) is True for k in ("pixels_required", "retrieval_required", "physical_qc_required", "visual_qc_required", "evidence_required", "pass_requires_all")):
        return fail("INCOMPLETE_EVIDENCE_CONTRACT")

    # The repository contract uses logical asset paths. Materialization is an
    # execution concern, so accept an override for worker-mounted roots.
    root = Path(sys.environ["TRIPPEDD_ASSET_ROOT"]) if "TRIPPEDD_ASSET_ROOT" in sys.environ else Path(".")
    ...
    if not any(p.exists() for p in mars_candidates):
        return fail("MARS_CANONICAL_PAYLOAD_NOT_MATERIALIZED", {
            "logical_id": "MARS_CANONICAL",
            "checked": [str(p) for p in mars_candidates],
        })

    if not env_path.is_file():
        return fail("ENVIRONMENT_PAYLOAD_NOT_MATERIALIZED", {
            "logical_id": required_world,
            "path": str(env_path),
        })

    if manifest.get("evidence", {}).get("gate") != "BLOCKED_UNTIL_REAL_RENDER":
        return fail("MANIFEST_GATE_TAMPERED", {
            "expected": "BLOCKED_UNTIL_REAL_RENDER",
            "actual": manifest.get("evidence", {}).get("gate"),
        })

    print(json.dumps({
        "status": "RESOLVED",
        "character": required_character,
        "world": required_world,
        "world_seed": 742918,
        "environment": str(env_path),
        "execution": "Blender",
        "next": "RUN_REAL_RENDER",
        "evidence": "NOT_ATTEMPTED",
