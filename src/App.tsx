...
import { PilotBuildWorkspace } from './components/PilotBuildWorkspace';
...
export default function App() {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'studio_ops': return <StudioOpsWorkspace />;
      ...
      case 'bible': return <ShowBible />;
      case 'ailab': return <AILab />;
      case 'tool_manager': return <ToolManager />;
      case 'episodes': return <EpisodeWorkspace />;
      case 'ingest': return <DriveIngestWorkspace />;
      ...
      case 'tool_2d':
      case 'settings':
        return <div className="flex-1 flex flex-col items-center justify-center text-neutral-500 h-full p-8 text-center min-h-full"><Wrench size={48} className="mb-4 opacity-20" /><h2 className="text-xl font-bold text-neutral-400 mb-2 font-mono tracking-widest uppercase">Integration Not Implemented</h2><p className="text-sm max-w-md">The {currentView.replace('tool_', '')} module is pending implementation in the Tool Registry.</p></div>;
      default: return <div className="flex-1 flex items-center justify-center text-neutral-500 font-mono text-sm min-h-full">Module [{currentView}] in development...</div>;
    }
  };
