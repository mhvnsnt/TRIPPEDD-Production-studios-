...
function Viewport({ exposure, gamma, pass, engine, truth }: { exposure: number; gamma: number; pass: RenderPass; engine: Engine; truth: string }) {
  return <div className="viewport">
    <div className="scanline" /><div className="mars-orb"><div className="mars-highlight" /><div className="mars-shadow" /><div className="mars-ring" /></div><div className="viewport-grid" />
    ...
    <div className="viewport-truth">{truth}</div>
  </div>;
}
...
export function RocketStudio() {
  ...
  const [renderTruth, setRenderTruth] = useState('PREVIEW VISUALIZATION · AUTHORITATIVE PIXELS: UNAVAILABLE');
  ...
  const startFrameRender = async () => {
    setRenderState('QUEUED');
    setRenderTruth('RENDER REQUEST SENT · WAITING FOR PHYSICAL WORKER + PROVENANCE');
    try {
      setRenderState('RUNNING');
      const response = await fetch('/api/rocket/render', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pass, engine, resolution, samples, exposure, gamma, camera, focalLength, aperture })
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.reason || `Render bridge returned ${response.status}`);
      if (result.authoritativePixels) {
        setRenderState('COMPLETED');
        setRenderTruth(`AUTHORITATIVE PIXELS · SHA256 ${String(result.artifactSha256).slice(0, 16)}… · PROVENANCE VERIFIED`);
      } else {
        setRenderState(result.status || 'AWAITING_PROVENANCE');
        setRenderTruth('WORKER RESPONSE RECEIVED · AUTHORITATIVE PIXELS REQUIRE ARTIFACT BYTES + SHA256 + PROVENANCE');
      }
    } catch (error) {
      setRenderState('BLOCKED');
      setRenderTruth(`RENDER BLOCKED · ${error instanceof Error ? error.message : 'worker unavailable'} · NO SIMULATED EVIDENCE`);
    }
  };
  ...
  return <main className="rocket-shell">
    ...
    {workspace === 'render' ? <section className="workspace">
      <aside className="control-panel">
        ...
        <div className="truth-card"><b>RENDER AUTHORITY</b><span>Viewport is an interactive control surface. No authoritative frame is claimed until a real render worker returns artifact bytes, SHA256, and provenance.</span></div>
        ...
        <div className="camera-advanced"><Slider label="Focal length" value={focalLength} min={18} max={120} step={1} onChange={setFocalLength} /><Slider label="Aperture" value={aperture} min={0.7} max={16} step={0.1} onChange={setAperture} /></div>
        <button className="primary" onClick={startFrameRender}>RENDER FRAME → {renderState}</button>
      </aside>
      <div className="render-main"><Viewport exposure={exposure} gamma={gamma} pass={pass} engine={engine} truth={renderTruth} /><div className="render-footer"><span>CAMERA {camera}</span><span>{resolution}% RES</span><span>{samples} SAMPLES</span><span>WORKER {renderState === 'COMPLETED' ? 'AUTHORITATIVE' : renderState === 'BLOCKED' ? 'BLOCKED' : 'PHYSICAL'}</span></div></div>
