...
export interface ComedySignal {
  id: string;
  type: ComedySignalType;
  score: number;
  startTime?: number;
  endTime?: number;
  evidence: string;
  source: 'TRANSCRIPT' | 'SCENE' | 'VISUAL' | 'METADATA';
}

export interface GagCandidate {
  id: string;
  title: string;
  score: number;
  signals: ComedySignal[];
  tags: string[];
  callbackKeys: string[];
  reviewState: 'MACHINE_SUGGESTED' | 'HUMAN_ACCEPTED' | 'HUMAN_REJECTED';
}

const clamp = (n: number) => Math.max(0, Math.min(1, n));

export function discoverComedy(input: {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const signals: ComedySignal[] = [];

    if (gap >= 2.0) signals.push({ id: randomUUID(), type: 'AWKWARD_SILENCE', score: clamp(gap / 6), startTime: previous?.end, endTime: current.start, evidence: `${gap.toFixed(1)}s speech gap`, source: 'TRANSCRIPT' });
    ...
    if (signals.length) {
      const score = clamp(signals.reduce((sum, signal) => sum + signal.score, 0) / signals.length + (signals.length > 1 ? 0.12 : 0));
      candidates.push({
        id: randomUUID(),
        title: text.length > 80 ? `${text.slice(0, 77)}...` : text,
        score,
        signals,
        tags: [...new Set(signals.map(signal => signal.type.toLowerCase()))],
        callbackKeys: extractCallbackKeys(text),
  ...
  if (ocr) {
    candidates.push({
      id: randomUUID(),
      title: 'On-screen text / prop opportunity',
      score: 0.45,
      signals: [{ id: randomUUID(), type: 'VISUAL_GAG', score: 0.45, evidence: ocr.slice(0, 500), source: 'VISUAL' }],
      tags: ['ocr', 'visual-gag'],
      callbackKeys: extractCallbackKeys(ocr),
      reviewState: 'MACHINE_SUGGESTED',
    });
  }
