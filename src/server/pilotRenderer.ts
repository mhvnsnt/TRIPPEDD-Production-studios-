...
function interleaveGenerated(clips: PilotAssemblyClip[], generated: PilotAssemblyGeneratedClip[]) {
  const beforeFinal = generated.filter(item => item.position === 'BEFORE_FINAL_SOURCE_CLIPS');
  const terminal = generated.filter(item => item.position === 'TERMINAL_TAG');
  const insertAt = Math.max(0, clips.length - 3);
  const ordered: Array<PilotAssemblyClip | PilotAssemblyGeneratedClip> = [...clips.slice(0, insertAt), ...beforeFinal, ...clips.slice(insertAt), ...terminal];
  return ordered;
}
...
function writeOtioTimeline(clips: PilotAssemblyClip[], generated: PilotAssemblyGeneratedClip[]) {
  ...
  const otioClips = interleaveGenerated(clips, generated).map((item, index) => {
    if ('sourceFileId' in item) {
      const durationFrames = Math.max(1, Math.round((item.end - item.start) * rate));
      const clip = item;
      const result = {
    ...
    return {
      OTIO_SCHEMA: 'Clip.2', name: item.id, source_range: null,
      media_reference: { OTIO_SCHEMA: 'ExternalReference.1', target_url: item.path, available_range: null, metadata: { trippedd: { provenance: item.provenance, purpose: item.purpose } } },
      metadata: { trippedd: { timelineStartFrame: timelineFrame, physicalTruth: false, generated: true, purpose: item.purpose, position: item.position, cutMode: CUT_MODE } }
    } as any;
  });
...
export async function buildEp01FirstAssembly(options: { maxClips?: number; clipPaddingSeconds?: number } = {}): Promise<PilotAssemblyManifest> {
  ...
  if (await fs.access(GENERATED_SUBJECTIVITY).then(() => true).catch(() => false)) generatedClips.push({ id: 'ep01-lost-acid-subjectivity', path: GENERATED_SUBJECTIVITY, purpose: "Audience sees the character's subjective experience before returning to live action; generated material is not physical source evidence.", provenance: 'GENERATED', position: 'BEFORE_FINAL_SOURCE_CLIPS' });
  if (await fs.access(GENERATED_BASTARD_TAG).then(() => true).catch(() => false)) generatedClips.push({ id: 'ep01-bastard-tag', path: GENERATED_BASTARD_TAG, purpose: 'Terminal mystery tag introducing Bannon/The Bastard after The Walk. This is generated and not physical source evidence.', provenance: 'GENERATED', position: 'TERMINAL_TAG' });
  ...
  const insertAt = Math.max(0, selected.length - 3);
  const orderedPaths = [
    ...sourceSegmentPaths.slice(0, insertAt),
    ...beforeFinal.map(item => generatedSegmentPaths.get(item.id)!).filter(Boolean),
    ...sourceSegmentPaths.slice(insertAt),
    ...terminal.map(item => generatedSegmentPaths.get(item.id)!).filter(Boolean)
  ];
