...
export function Sidebar({ currentView, setCurrentView }: { currentView: string, setCurrentView: (v: string) => void }) {
  ...
  const NavGroup = ({ title, items }: any) => (
    <div className="mb-6">
      {!isCollapsed && <div className="text-[10px] font-bold text-neutral-600 mb-2 tracking-widest px-3 uppercase">{title}</div>}
        ...
        {items.map((item: any) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center px-3 py-2.5 rounded-md transition-colors relative group ${
                isActive 
  ...
  const toolItems = [
    { id: 'tool_video', label: 'Video Edit', icon: Clapperboard },
    { id: 'tool_comfy', label: 'ComfyUI', icon: Layers },
    { id: 'tool_blender', label: 'Blender', icon: MonitorPlay },
    { id: 'tool_unreal', label: 'Unreal', icon: MonitorPlay },
    { id: 'tool_audio', label: 'Audio', icon: Mic },
    { id: 'tool_capture', label: 'Capture', icon: Video },
    { id: 'tool_2d', label: '2D / Graphics', icon: ImageIcon },
  ];

  const systemItems = [
    { id: 'jobs', label: 'Jobs & Pipeline', icon: Activity },
    { id: 'tool_manager', label: 'Tool Manager', icon: Wrench },
  ...
  return (
    <motion.div 
      ...
      className="h-screen bg-neutral-900 border-r border-neutral-800 flex flex-col text-neutral-300 relative z-20 shrink-0"
    >
      <div className={`p-4 border-b border-neutral-800 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
        {!isCollapsed && (
          <h1 className="text-xl font-black tracking-tighter text-white uppercase flex flex-col overflow-hidden">
            <span>TRIPPEDD</span>
          </h1>
        )}
        <button 
      ...
      <nav className="flex-1 py-4 px-2 overflow-y-auto overflow-x-hidden custom-scrollbar">
